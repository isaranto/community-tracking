...
class TensorFact:
    ...
    def create_dense_tensor(self, graphs):
        n = 0
        nodes_id = {}
    ...
    def get_non_empties(self, graphs):
        ...
        T = sptensor(tuple(triplets.T), vals=np.ones(len(triplets)), shape=(len(self.node_ids), len(self.node_ids),
                                                                            len(graphs)), dtype=int)
        return T

    def tensor_decomp(self, num_of_coms):
        np.random.seed(4)
        X_approx_ks = ncp.nonnegative_tensor_factorization(self.tensor, num_of_coms, method='anls_bpp', stop_criterion=2)
        A = X_approx_ks.U[0]
        B = X_approx_ks.U[1]
        C = X_approx_ks.U[2]
        return A, B, C
